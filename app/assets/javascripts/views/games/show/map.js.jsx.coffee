...
Unit = React.createClass
  render: ->
    ...
    coords = new Vector unit_coords @props.coords, unit
...
Polygon = React.createClass
  render: ->
    ...
    options.id = "#{@props.region}_#{polygon.part}" if polygon.part
...
Region = React.createClass
  render: ->
    ...
    coords.polygons.forEach (polygon, index)->
      polygons.push `<Polygon key={index} region={name} polygon={polygon} />`
...
vr.MapContainer = React.createClass
  getInitialState: ->
  ...
  render: ->
    ...
    `<div className={className}>
      <Map state={this.state.state} coords={this.props.coords} />
