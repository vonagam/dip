module Diplomacy
  class Parser::State
    ...
    def dump_state
      ...
      @gamestate.each do |area, area_state|
        unless area_state.unit.nil?
          ...
          full_area = "#{area}#{ area_state.coast.nil? ? "" : "_#{area_state.coast}"}"
          powers[nationality][:units][full_area] = area_state.unit.type_to_s
        end
    ...
    def parse_unit(blob, power)
      m = /(?'unit_type'[AF])(?'unit_area'\w{3})(_(?'unit_area_coast'\w{2}))?(\*(?'dislodge_origin'\w{3}))?/.match(blob)
